...
const scriptPath = path.resolve('scripts/sync-bambuddy-stable.mjs');
const validDigest = 'sha256:c670164aaa3b0c5af715ca00e9745cdf3a4d7d337fc11d96fc85180371952698';

function withPackage(run) {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'bambuddy-stable-sync-'));
...
test('writes an immutable stable channel descriptor', () => {
  withPackage((directory) => {
    runSync(directory, '1.2.5.6', validDigest);
    const channel = JSON.parse(
      fs.readFileSync(path.join(directory, 'channels/bambuddy/stable.json'), 'utf8'),
    );
    assert.equal(channel.channel, 'stable');
    assert.equal(channel.version, '1.2.5.6');
    assert.equal(channel.releaseTag, 'v1.2.5.6');
    assert.equal(channel.image, 'ghcr.io/maziggy/bambuddy:1.2.5.6');
    assert.equal(channel.digest, validDigest);
    assert.equal(
      channel.immutableImage,
      `ghcr.io/maziggy/bambuddy:1.2.5.6@${validDigest}`,
    );
...
test('accepts three- and four-component stable versions', () => {
  for (const version of ['1.2.6', '1.2.5.6']) {
    withPackage((directory) => {
...
test('rejects prerelease, mutable-only and malformed versions', () => {
  for (const version of ['1.2.6b1', 'daily', 'v1.2.6', '1.2', '1.2.6-daily.20260830']) {
    withPackage((directory) => {
      assert.throws(
        () => runSync(directory, version, validDigest),
