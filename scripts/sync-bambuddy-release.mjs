...
if (!/^\d+\.\d+\.\d+(?:\.\d+)?$/.test(version ?? '') || /(?:beta|daily|nightly|dev|main)/i.test(version)) {
...
if (!imagePattern.test(compose)) {
  throw new Error('Bambuddy image reference was not found in docker-compose.yml');
}
if (!/^version: "\d+\.\d+\.\d+(?:\.\d+)?"$/m.test(manifest)) {
...
const nextManifest = manifest
  ...
  .replace(
    /releaseNotes: >-[\s\S]*$/m,
    `releaseNotes: >-\n  Автоматическое обновление Bambuddy до ${version}.\n  Данные и настройки приложения сохраняются.`,
  );
