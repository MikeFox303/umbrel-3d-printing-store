...
restart_app() { umbrel_cmd apps.restart.mutate --appId "$1"; }
...
if isinstance(value, dict):
    for key in ("items", "printers", "data"):
        if isinstance(value.get(key), list):
            value = value[key]
            break
printers = value if isinstance(value, list) else []
...
for item in enabled:
    ...
    print(f"  VP id={item.get('id')} name={item.get('name')} mode={item.get('mode')} running={runtime.get('running')}")
...
for cmd in docker python3 curl umbreld; do need "$cmd"; done
